	...
	jv_grid_populated_sidexside_demo
		...
		do
			...
			create l_other
				...
				-- Custom Enhancements
			l_item.enable_row_colorizing
			l_item.enable_enter_key_tabbing

				-- Build
			build_complex_grid (l_item)
			build_complex_grid (l_other)

				-- Setup and Demo
			create l_container
			l_container.extend (l_item)
			l_container.extend (l_other)
...
feature {NONE} -- Implementation: Grid Setup
	...
	build_complex_grid (l_item: EV_GRID)
			--
		local
